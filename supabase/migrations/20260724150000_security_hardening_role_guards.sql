-- HARDENING (auditoria R4/scan Lovable, consenso Claude+Codex):
-- 1) 9 RPCs SECURITY DEFINER granted a authenticated ganham guard de ROLE
--    (trainer OU admin). Modelo Fabrik: estudio compartilhado (Alex+Felipe veem
--    tudo) -> o risco real e CONTA DE ALUNO chamando as RPCs, nao treinador
--    vendo treinador; por isso guard por role, nao por dono (divergencia
--    consciente da sugestao de escopo por trainer do Codex).
-- 2) 6 policies de tabelas sensiveis recriadas TO authenticated (eram role
--    public) com o MESMO owner-check + WITH CHECK + bypass de admin.
-- Funcoes LANGUAGE sql: guard via public.assert_staff() como 1a statement
-- (multi-statement SQL function retorna a ultima). Corpos originais intactos.
...
-- guard: count_students_frequency_dropping
...
-- guard: count_prescriptions_stagnant
...
-- guard: list_students_frequency_dropping
...
-- guard: list_prescriptions_stagnant
...
-- guard: list_unlinked_session_exercise_review
...
-- ===================== POLICIES: TO authenticated + WITH CHECK =====================
...
DROP POLICY IF EXISTS "Trainers manage athlete_records" ON public.athlete_records;
CREATE POLICY "Trainers manage athlete_records" ON public.athlete_records
  FOR ALL TO authenticated
  USING ( public.has_role(auth.uid(), 'admin') OR (EXISTS (
...
DROP POLICY IF EXISTS "Trainers manage athlete_goals" ON public.athlete_goals;
CREATE POLICY "Trainers manage athlete_goals" ON public.athlete_goals
  FOR ALL TO authenticated
  USING ( public.has_role(auth.uid(), 'admin') OR (EXISTS (
      SELECT 1 FROM public.students s
...
-- ===================== BACKFILL DE ROLES (fecha gap pré-existente) =====================
-- user_roles tinha 1 row (admin=Alex); treinadores ativos sem row (Felipe) já
-- estavam bloqueados do generate-group-session (mesmo role-check) e ficariam
-- fora das RPCs guardadas. Backfill: todo trainer com alunos ganha 'trainer'.
INSERT INTO public.user_roles (user_id, role)
SELECT DISTINCT s.trainer_id, 'trainer'::public.app_role
FROM public.students s
WHERE s.trainer_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM public.user_roles ur WHERE ur.user_id = s.trainer_id
  )

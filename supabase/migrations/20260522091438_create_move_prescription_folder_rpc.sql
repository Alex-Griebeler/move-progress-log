...
--
-- The existing trigger trigger_update_folder_full_path only recomputes the
-- row it fires on. When a folder is re-parented, its children/grandchildren
-- also need depth_level/full_path recomputed. This RPC is the single, safe
-- authority for moving a folder: it validates ownership and the move, then
-- recalculates the whole moved subtree inside one transaction.
--
-- SECURITY INVOKER (default): runs as the authenticated caller, so RLS on
-- prescription_folders ("Trainers manage own folders") applies on top of the
-- explicit ownership checks below. Frontend guards are UX-only; this function
...
BEGIN
  ...
  -- The folder being moved must exist and belong to the current trainer.
  ...
  -- When a destination parent is given it must exist and belong to the
  -- same trainer; null means "move to root".
  IF p_new_parent_id IS NOT NULL THEN
    ...
    -- Cannot move a folder into one of its own descendants (cycle guard).
    IF EXISTS (
  ...
  -- Depth guard across the WHOLE moved subtree. The moved folder lands at
  -- v_new_depth; its deepest descendant ends at v_new_depth + subtree height.
  ...
  -- Resolve order_index: explicit value, otherwise append after the
  -- destination's existing siblings.
  ...
  -- Move the folder. The BEFORE UPDATE trigger trigger_update_folder_full_path
  -- recomputes depth_level/full_path for THIS row.
  UPDATE prescription_folders
  SET parent_id = p_new_parent_id,
      order_index = v_order
  WHERE id = p_folder_id;

  -- Recalculate depth_level/full_path for the whole subtree below the moved
  -- folder. These updates touch neither name nor parent_id, so the trigger
  -- does not fire for them -- the RPC is the explicit authority here.
  WITH RECURSIVE subtree AS (
    SELECT c.id,
...
-- EXECUTE only for authenticated users; never anon.

...
-- Unifica sinonimos/plurais/typos no token dominante, SEM perder distincoes reais.
-- Merges: isquiotibiais->isquiotibial, latissimo_dorso->latissimo,
-- tensor_fascia_lata->TFL, gluteo_maximo->gluteo, peitoral_maior->peitoral,
-- biceps_femoral->isquiotibial, panturrilha->gastrocnemio, reto_femoral->quadriceps,
-- iliopsoas->flexor_quadril, virilha->adutores. Mantidos distintos: deltoide
-- generico, soleo, VMO, gluteo_medio, cabecas do deltoide, peitoral_menor/inferior,
-- trapezio_inferior. Whole-array por exercicio, dedup preservando ordem.
-- Idempotente (IS DISTINCT FROM ... ::text[], casa por id8+name). Consenso Claude+Codex.

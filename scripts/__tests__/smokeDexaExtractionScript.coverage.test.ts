...
describe("smoke-dexa-extraction.mjs — micro-hardening", () => {
  ...
  it("file_data usa DATA URL (espelha o fix da edge — base64 puro foi rejeitado)", () => {
    // O smoke real pós PR #162 (com `failure_code` exposto) confirmou
    // que `file_data: <base64 puro>` é rejeitado pela Responses API
    // com HTTP 400 / error.code="invalid_value". Voltamos pra data URL.

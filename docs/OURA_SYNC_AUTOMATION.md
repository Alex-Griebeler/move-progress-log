...
0. **Migration `20260912150000_oura_upsert_merge_rpc.sql` ANTES do `oura-sync`** (revisão do merge atômico): o `oura-sync` novo grava só pelo RPC `upsert_oura_row_merge`; sem a função, toda gravação de métricas responde 500 ("function … does not exist") e nada é salvo. Depois de aplicar, rodar `NOTIFY pgrst, 'reload schema';` (existir em `pg_proc` NÃO prova que a API já enxerga a função: o PostgREST tem cache de schema) e confirmar pela API antes de publicar a edge: com o `oura-sync` antigo ainda no ar, a prova é uma chamada direta ao RPC pela API REST com a chave de service role — `POST /rest/v1/rpc/upsert_oura_row_merge` com body `{"p_table":"students","p_row":{}}` tem de responder **400 com a mensagem "tabela não permitida"** (a função rodou e recusou); 404 "Could not find the function" = cache ainda velho, repetir o NOTIFY e esperar. Só então publicar `oura-sync` e conferir um sync manual de 1 dia respondendo 200.

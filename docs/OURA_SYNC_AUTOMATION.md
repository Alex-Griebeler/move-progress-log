...
- **Lookback:** cada execução do `oura-sync-all` (cron ou botão "Sincronizar Todos") sincroniza **hoje, ontem e anteontem** (`lookback_days`, padrão 2, máximo 6 — só configurável em chamada manual com service role/admin; o cron e a UI usam o padrão), em sequência por aluna. Hoje tem 3 tentativas; dias anteriores, 2. **Orçamento global de 110 s por execução** (idle timeout da edge function = 150 s): o plano roda "hoje" para todas as alunas antes de "ontem"; passos que não cabem são pulados (`status: skipped`, `truncated: true` na resposta) e o cron seguinte os revisita. Timeouts das chamadas ao Oura via `AbortSignal.timeout` (cancelam requisição e leitura do corpo). Scores que o Oura finaliza depois da última execução do dia são recuperados na execução seguinte; o upsert preserva valores já gravados quando o payload vem esparso.

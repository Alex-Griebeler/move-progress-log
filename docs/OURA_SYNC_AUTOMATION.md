...
- **Cron versionado (migration `20260909180000`):** remove por jobid todo job cujo comando INTEIRO é uma invocação reconhecida do `oura-sync-scheduled` (regex ancorado: `SELECT private.invoke_cron_edge('oura-sync-scheduled', …)` ou `SELECT net.http_post('https://…/functions/v1/oura-sync-scheduled', …)`) e recria os três (9h/13h/21h UTC). Um job que apenas mencione a string (comentário, monitoramento, rota no body de um http_post para outra função) **aborta a migration** listando o jobid, para decisão humana. `truncated` na resposta do `oura-sync-all` só é verdadeiro quando algum par ficou sem desfecho, não quando o deadline pegou a execução apenas esperando um insert de log.

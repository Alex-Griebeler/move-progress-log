...
- **Cron versionado (migration `20260909180000`):** remove por jobid todo job que **invoque** o `oura-sync-scheduled` em uma das duas formas conhecidas (`private.invoke_cron_edge('oura-sync-scheduled', …)` ou `net.http_post(…/functions/v1/oura-sync-scheduled…)`) e recria os três (9h/13h/21h UTC). Um job que apenas mencione a string sem invocar **aborta a migration** listando o jobid, para decisão humana.

...
describe("getTrainingAlternativesForZone", () => {
  it("cada uma das 5 zonas do motor cai no bloco certo de alternativas", () => {
    expect(getTrainingAlternativesForZone("green_high", 0)[0].type).toMatch(/Desafio Máximo/i);
    expect(getTrainingAlternativesForZone("green", 0)[0].type).toMatch(/Treino Completo/i);
    expect(getTrainingAlternativesForZone("yellow", 0)[0].type).toMatch(/Redução Moderada/i);
  ...
  it("zona rebaixada por override mostra alternativas da zona FINAL, não do score", () => {
    ...
    expect(alts[0].type).toMatch(/Treino Completo/i);
    expect(alts[0].type).not.toMatch(/Desafio Máximo/i);
  });

  it("sem recomendação (fonte Whoop nesta fase): fallback pelo score", () => {
    expect(getTrainingAlternativesForZone(null, 90)[0].type).toMatch(/Desafio Máximo/i);
    expect(getTrainingAlternativesForZone(null, 50)[0].type).toMatch(/Redução Moderada/i);
  });
});
